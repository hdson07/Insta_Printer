    ...
    while True:
        ...
        if isinstance(ele_imgs, list):
            for ele_img in ele_imgs:
                for i in range(0,15) : 
                    if ele_img.get_attribute("src") is not None : 
                        img_urls.add(ele_img.get_attribute("src"))        
                        break
                    else :
                        sleep(2)
                        print("WIFI is unstable, retry get url")
                    if i == 14 :
                        print("WIFI is disconnected, Please Check your WIFI and Restart program")
                        img_urls.add("no_wifi.png")
        else:
            break
    ...
    if len(ele_comments) > 0:
        ...
        for element in temp_element:
            if element.text not in ['Verified',''] :
                dict_post["caption"] = dict_post["caption"] + element.text + " "
